...
function initNetworkObserver(
  ...
  const callback: networkCallback = (data) => {
  ...
  const performanceObserver = initPerformanceObserver(callback, win, networkOptions);
  const xhrObserver = initXhrObserver(callback, win, networkOptions);
  const fetchObserver = initFetchObserver(callback, win, networkOptions);
  return () => {
    performanceObserver();
    xhrObserver();
    fetchObserver();
